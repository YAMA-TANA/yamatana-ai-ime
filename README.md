...
> **Beta / 未署名** — `v0.1.3-beta` は検証用の未署名Betaです。Windowsの警告が表示される場合があります。現在はSignPath Foundationによる署名を申請する前段階であり、署名済みであるかのような表示は行いません。
...
2. PowerShellで `Get-FileHash .\Yamatana-AI-IME-MOZC-Ver-0.1.3-beta-x64.msi -Algorithm SHA256` を実行し、公開ハッシュと一致することを確認します。
