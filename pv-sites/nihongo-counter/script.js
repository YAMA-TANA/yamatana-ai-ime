...
function updateCounts() {
  ...
  const all = graphemeCount(value);
  const compact = graphemeCount(withoutWhitespace);
  countAll.textContent = all.toLocaleString(locale);
  countNoSpace.textContent = compact.toLocaleString(locale);
  countLines.textContent = value ? String(value.split(/\r\n|\r|\n/).length) : '0';
  countPages.textContent = compact ? String(Math.ceil(compact / 400)) : '0';
}

input.addEventListener('input', () => { input.dataset.userEdited = '1'; status.classList.remove('is-success'); updateCounts(); });
document.querySelector('#clearButton').addEventListener('click', () => { input.value = ''; input.dataset.userEdited = '1'; updateCounts(); input.focus(); status.classList.remove('is-success'); status.textContent = currentCopy().clearStatus; });
document.querySelector('#copyButton').addEventListener('click', async () => {
  if (!input.value) { status.classList.remove('is-success'); status.textContent = currentCopy().noCopy; return; }
  try { await navigator.clipboard.writeText(input.value); status.textContent = currentCopy().copied; status.classList.add('is-success'); } catch { status.classList.remove('is-success'); status.textContent = currentCopy().copyFailed; }
});
